...
class Cidades(SearchDomain):    # Deriva de search domain
    ...
    def actions(self,city):
        ...
        for (C1,C2,D) in self.connections:
            if (C1==city):
               ...
               actlist += [(C2,C1)]
        return actlist 
    def result(self,city,action): # Dada uma cidade, onde é que eu vou parar
        (C1,C2) = action
        if C1==city:
            return C2
    ...
    def heuristic(self, city, goal_city):
        # Aproximação
        # A melhor aprximação é a distância em linha reta entre uma dada cidade e o objetivo (goal)
        p = self.coordinates[city]
        q = self.coordinates[goal_city]
...
p = SearchProblem(cidades_portugal,'Braga','Faro')  # Criamos o problema
t = SearchTree(p,'depth')                           # Criamos uma árvore com o problema e o algoritmo desejado
...
p = SearchProblem(cidades_portugal,'Braga','Faro')  
t = SearchTree(p,'uniform')                     # Demora mais tempo, mas tem o menor custo                     
print(t.search(), t.cost)
